...
export const junior047: Mission = {
  id: "junior-ticket-047",
  world: "Northgate Bank",
  levelLabel: "Ticket JDA-047 // Final Assessment 3 of 6",
  npc: "SENIOR ANALYST KAVYA RATHI",
  briefing: [
    "11:00 AM. Client: Northgate Bank. Priority: Critical.",
    "\"Find the single largest transaction on record — but only consider customers who've made more than one transaction. A one-off customer with a lucky big deposit isn't the pattern we're looking for.\"",
  ],
  objective:
    "Among customers with more than one transaction, find the customer name and amount of the single largest transaction.",
  ...
  expectedRows: [["Ishita Ghosh", 80000]],
  requireRowOrder: false,
  hints: [
    "Two separate problems here: figuring out who qualifies (more than one transaction), and finding the biggest amount among only those people.",
    "Solve the qualification piece first as its own building block, then bring the full transaction/account/customer join in afterward, restricted to just those customers.",
    "Try: WITH cust_txn_counts AS (SELECT a.customer_id, COUNT(*) AS txn_count FROM transactions t JOIN accounts a ON t.account_id = a.id GROUP BY a.customer_id), multi AS (SELECT customer_id FROM cust_txn_counts WHERE txn_count > 1) SELECT c.name, t.amount FROM transactions t JOIN accounts a ON t.account_id = a.id JOIN customers c ON a.customer_id = c.id WHERE a.customer_id IN (SELECT customer_id FROM multi) ORDER BY t.amount DESC LIMIT 1;",
  ],
  xpAward: 250,
};
