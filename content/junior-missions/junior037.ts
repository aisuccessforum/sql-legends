...
export const junior037: Mission = {
  id: "junior-ticket-037",
  world: "Northgate Bank",
  levelLabel: "Ticket JDA-037 // Priority: High",
  npc: "SENIOR ANALYST KAVYA RATHI",
  briefing: [
    "10:20 AM. Client: Northgate Bank. Priority: High.",
    "\"New table this time — transactions, linked to accounts, which are linked to customers. Three tables, one chain. Show every transaction with the customer's name and which type of account it happened on.\"",
    "\"You've joined two tables plenty of times. Three is the exact same idea, one more JOIN clause.\"",
  ],
  objective:
    "Select the customer name, account type, transaction type, and amount for every transaction.",
  ...
  expectedColumns: ["name", "account_type", "type", "amount"],
  expectedRows: [
    ["Meera Chandrasekaran", "Savings", "deposit", 5000],
    ["Meera Chandrasekaran", "Savings", "withdrawal", -2000],
    ["Meera Chandrasekaran", "Current", "deposit", 3000],
    ["Tanya Kapoor", "Savings", "deposit", 10000],
    ["Rajiv Menon", "Current", "deposit", 50000],
    ["Rajiv Menon", "Current", "withdrawal", -15000],
    ["Sneha Pillai", "Savings", "deposit", 2000],
    ["Ishita Ghosh", "Savings", "deposit", 80000],
    ["Ishita Ghosh", "Savings", "deposit", 20000],
  ],
  requireRowOrder: false,
  hints: [
    "Give all three tables an alias — t for transactions, a for accounts, c for customers.",
    "Chain two JOIN clauses: transactions to accounts on account_id, then accounts to customers on customer_id.",
    "Try: SELECT c.name, a.account_type, t.type, t.amount FROM transactions t JOIN accounts a ON t.account_id = a.id JOIN customers c ON a.customer_id = c.id;",
  ],
  xpAward: 200,
};
