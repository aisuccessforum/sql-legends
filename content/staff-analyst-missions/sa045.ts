...
export const sa045: Mission = {
  id: "sa-ticket-045",
  world: "AstraMind Analytics",
  levelLabel: "Ticket SA-045 // Capstone Assessment",
  npc: "VP OF DATA SCIENCE ANIKA SHARMA",
  briefing: [
    "08:00 AM. Capstone Assessment begins.",
    "\"You've completed six modules. This is the final series \u2014 seven questions across all datasets. Answer every one correctly and you'll have demonstrated the analysis skills of a Staff Analyst at AstraMind.\"",
    "\"First question: statistical profile of the sales team. Mean, stddev, CV, and the name of the top performer \u2014 in one row.\"",
  ],
  objective:
    "In a single row, show sales team mean revenue (0 decimals), population stddev (0 decimals), coefficient of variation (1 decimal %), and the name of the rep with the highest revenue.",
...
`,
  ...
  expectedColumns: ["mean_rev", "stddev_rev", "cv_pct", "top_performer"],
  expectedRows: [
    [72500, 22420, 30.9, "Grace"],
  ],
  requireRowOrder: false,
  hints: [
    "Use a CTE for stats (mean, stddev, CV from sales_reps) and a second CTE for top performer name (WHERE revenue = MAX subquery). Cross-join in the SELECT.",
    "Try: WITH s AS (SELECT ROUND(AVG(revenue),0) AS mean_rev, ROUND(SQRT(SUM(revenue*revenue)*1.0/COUNT(*)-AVG(revenue)*AVG(revenue)),0) AS stddev_rev, ROUND(100.0*SQRT(...)/AVG(revenue),1) AS cv_pct FROM sales_reps), top AS (SELECT rep_name FROM sales_reps WHERE revenue=(SELECT MAX(revenue) FROM sales_reps)) SELECT s.mean_rev, s.stddev_rev, s.cv_pct, t.rep_name FROM s, top t;",
  ],
  xpAward: 350,
};
