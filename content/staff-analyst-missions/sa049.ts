...
export const sa049: Mission = {
  id: "sa-ticket-049",
  world: "AstraMind Analytics",
  levelLabel: "Ticket SA-049 // Capstone Assessment",
  npc: "VP OF DATA SCIENCE ANIKA SHARMA",
  briefing: [
    "\"Fifth question: customer revenue concentration with Pareto tier.\"",
    "\"Rank all 20 customers by revenue, show the running cumulative share, and tag the top 20% as 'Top 20%'. This is the table Anika will use to argue for doubling down on Enterprise.\"",
  ],
  objective:
    "Show all customers ranked by total revenue with their cumulative revenue share (1 decimal %) and a Pareto tier label ('Top 20%' or 'Other 80%') \u2014 sorted by rank.",
...
`,
  ...
  expectedColumns: ["rank", "customer_id", "customer_name", "segment", "total_revenue", "cumulative_pct", "pareto_tier"],
  expectedRows: [
    [1, "CU15", "OmniPath", "Enterprise", 420000, 19.8, "Top 20%"],
    [2, "CU07", "GlobalNet", "Enterprise", 330000, 35.3, "Top 20%"],
    [3, "CU01", "Apex Corp", "Enterprise", 240000, 46.6, "Top 20%"],
    [4, "CU11", "KineticLab", "Enterprise", 210000, 56.5, "Top 20%"],
    [5, "CU19", "StreamLine", "Enterprise", 190000, 65.5, "Other 80%"],
    [6, "CU04", "DataDriven", "Enterprise", 175000, 73.7, "Other 80%"],
    [7, "CU13", "MetaPulse", "Mid-Market", 90000, 78, "Other 80%"],
    [8, "CU09", "InfoSys", "Mid-Market", 87500, 82.1, "Other 80%"],
    [9, "CU20", "TechForge", "Mid-Market", 84000, 86, "Other 80%"],
    [10, "CU05", "EdgeTech", "Mid-Market", 66000, 89.1, "Other 80%"],
    [11, "CU17", "QuantumJet", "Mid-Market", 60000, 92, "Other 80%"],
    [12, "CU06", "FocusGroup", "Mid-Market", 45000, 94.1, "Other 80%"],
    [13, "CU02", "BlueSky", "SMB", 36000, 95.8, "Other 80%"],
    [14, "CU12", "LeadWave", "SMB", 30000, 97.2, "Other 80%"],
    [15, "CU16", "PeakView", "SMB", 18000, 98, "Other 80%"],
    [16, "CU08", "HighPoint", "SMB", 12000, 98.6, "Other 80%"],
    [17, "CU14", "NodeFlow", "SMB", 10500, 99.1, "Other 80%"],
    [18, "CU03", "CloudBase", "SMB", 8500, 99.5, "Other 80%"],
    [19, "CU18", "RealEdge", "SMB", 6000, 99.8, "Other 80%"],
    [20, "CU10", "JumpStart", "SMB", 4500, 100, "Other 80%"],
  ],
  requireRowOrder: true,
  hints: [
    "ROW_NUMBER() OVER (ORDER BY total_revenue DESC) for rank. SUM() OVER (ORDER BY ...) for running total. Divide by SUM() OVER () for percentage. WHERE rn <= n/5 flags the top quintile.",
    "Try: WITH ranked AS (SELECT ..., ROW_NUMBER() OVER (ORDER BY total_revenue DESC) AS rn, COUNT(*) OVER () AS n, SUM(total_revenue) OVER (ORDER BY total_revenue DESC) AS running_rev, SUM(total_revenue) OVER () AS grand_total FROM customer_orders) SELECT ..., ROUND(100.0*running_rev/grand_total,1) AS cumulative_pct, CASE WHEN rn<=n/5 THEN 'Top 20%' ELSE 'Other 80%' END FROM ranked ORDER BY rn;",
  ],
  xpAward: 375,
};
