...
export const sa048: Mission = {
  id: "sa-ticket-048",
  world: "AstraMind Analytics",
  levelLabel: "Ticket SA-048 // Capstone Assessment",
  npc: "VP OF DATA SCIENCE ANIKA SHARMA",
  briefing: [
    "\"Fourth question: monthly revenue trend with regression line and residuals.\"",
    "\"All 12 months: actual revenue, predicted trend-line value, and the residual. The July dip should stand out as the largest negative residual. This is what you show in the board deck to explain the seasonal pattern.\"",
  ],
  objective:
    "For each month, show month_num, month_label, actual revenue, trend line value (0 decimals), and residual (0 decimals) \u2014 sorted by month_num.",
...
`,
  ...
  expectedColumns: ["month_num", "month_label", "revenue", "trend_line", "residual"],
  expectedRows: [
    [1, "Jan", 42000, 42154, -154],
    [2, "Feb", 45000, 44717, 283],
    [3, "Mar", 48000, 47280, 720],
    [4, "Apr", 50000, 49843, 157],
    [5, "May", 53000, 52406, 594],
    [6, "Jun", 56000, 54969, 1031],
    [7, "Jul", 55000, 57531, -2531],
    [8, "Aug", 59000, 60094, -1094],
    [9, "Sep", 62000, 62657, -657],
    [10, "Oct", 65000, 65220, -220],
    [11, "Nov", 68000, 67783, 217],
    [12, "Dec", 72000, 70346, 1654],
  ],
  requireRowOrder: true,
  hints: [
    "Two CTEs: agg for n/sx/sy/sxy/sx2, then p for slope+intercept. Join monthly_sales to params and compute trend_line = slope*month_num+intercept, residual = revenue-trend_line.",
    "Try: WITH agg AS (...), p AS (SELECT slope, intercept FROM agg params) SELECT m.month_num, m.month_label, m.revenue, ROUND(p.slope*m.month_num+p.intercept,0) AS trend_line, ROUND(m.revenue-(p.slope*m.month_num+p.intercept),0) AS residual FROM monthly_sales m, p ORDER BY m.month_num;",
  ],
  xpAward: 375,
};
