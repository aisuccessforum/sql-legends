...
export const sa032: Mission = {
  id: "sa-ticket-032",
  world: "AstraMind Analytics",
  levelLabel: "Ticket SA-032 // Priority: High",
  npc: "VP OF DATA SCIENCE ANIKA SHARMA",
  briefing: [
    "09:45 AM.",
    "\"IQR outliers \u2014 flag every order outside the Tukey fences. Two rows should appear: O005 (4800) and O012 (3900). These are the orders that warrant investigation before they distort any aggregate.\"",
  ],
  objective:
    "Return orders flagged as IQR outliers \u2014 showing order_id, customer_id, amount, and flag ('High Outlier' / 'Low Outlier') \u2014 sorted by amount descending.",
...
`,
  ...
  expectedColumns: ["order_id", "customer_id", "amount", "flag"],
  expectedRows: [
    ["O005", "C005", 4800, "High Outlier"],
    ["O012", "C002", 3900, "High Outlier"],
  ],
  requireRowOrder: true,
  hints: [
    "Extend the IQR CTE from the previous ticket with a fences CTE, then JOIN to order_amounts WHERE amount < lo OR amount > hi.",
    "Try: WITH nt/b/fences CTEs as before, SELECT o.order_id, o.customer_id, o.amount, CASE WHEN o.amount>f.hi THEN 'High Outlier' ... FROM order_amounts o, fences f WHERE o.amount<f.lo OR o.amount>f.hi;",
  ],
  xpAward: 250,
};
