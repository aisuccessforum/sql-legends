...
export const sa019: Mission = {
  id: "sa-ticket-019",
  world: "AstraMind Analytics",
  levelLabel: "Ticket SA-019 // Priority: High",
  npc: "VP OF DATA SCIENCE ANIKA SHARMA",
  briefing: [
    "10:30 AM.",
    "\"Revenue per user (ARPU) \u2014 average revenue including non-converters. Treatment ARPU higher than control means the feature lifts revenue even accounting for users who didn't buy. SUM(revenue)/COUNT(*) per variant.\"",
  ],
  objective:
    "For each variant, show user count, total revenue, and average revenue per user (2 decimals) \u2014 sorted by variant.",
...
`,
  ...
  expectedColumns: ["variant", "users", "total_revenue", "arpu"],
  expectedRows: [
    ["control", 10, 300, 30],
    ["treatment", 10, 600, 60],
  ],
  ...
  hints: [
    ...
    "Try: SELECT variant, COUNT(*) AS users, SUM(revenue) AS total_revenue, ROUND(SUM(revenue)/COUNT...",
  ],
  xpAward: 200,
};
