...
export const de033: Mission = {
  id: "de-ticket-033",
  world: "AstraMind Analytics",
  levelLabel: "Ticket DE-033 // Priority: High",
  npc: "PRINCIPAL DATA ENGINEER VIKRAM NAIR",
  briefing: [
    "09:45 AM. Internal system. Priority: High.",
    "\"Missing records — the row-count gap tells you there's a problem; the anti-join tells you exactly which records are missing. LEFT JOIN source to target, WHERE target key IS NULL.\"",
  ],
  objective:
    "Return orders in source_orders that are absent from target_orders — order_id, customer_id, amount, and created_at — sorted by order_id.",
  ...
  expectedRows: [
    ["ORD-009", "C002", 8000, "2026-03-18"],
    ["ORD-010", "C003", 4400, "2026-04-02"],
  ],
  requireRowOrder: true,
  hints: [
    "LEFT JOIN source_orders to target_orders on order_id, WHERE t.order_id IS NULL.",
    "Try: SELECT s.order_id, s.customer_id, s.amount, s.created_at FROM source_orders s LEFT JOIN target_orders t ON s.order_id=t.order_id WHERE t.order_id IS NULL ORDER BY s.order_id;",
  ],
  xpAward: 225,
};
