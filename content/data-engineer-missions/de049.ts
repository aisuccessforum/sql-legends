...
export const de049: Mission = {
  id: "de-ticket-049",
  world: "AstraMind Analytics",
  levelLabel: "Ticket DE-049 // Final Assessment 5 of 6",
  npc: "PRINCIPAL DATA ENGINEER VIKRAM NAIR",
  briefing: [
    "01:00 PM. Final Assessment.",
    "\"Cross-system health check — pipeline health across ETL, reconciliation, and dimension. Completed event revenue, unloaded orders, failed events in one query.\"",
  ],
  objective:
    "In a single row, show source order count, target order count, unloaded order count, total completed event revenue, and count of failed events.",
  ...
  expectedColumns: ["source_orders", "target_orders", "unloaded", "total_event_revenue", "failed_events"],
  expectedRows: [
    [10, 8, 2, 25400, 2],
  ],
  requireRowOrder: false,
  hints: [
    "Five scalar subqueries spanning raw_events, source_orders, and target_orders. Unloaded = anti-join count. Completed revenue uses CAST(COALESCE(revenue,'0') AS REAL).",
    "Try: SELECT (SELECT COUNT(*) FROM source_orders) AS source_orders, (SELECT COUNT(*) FROM target_orders) AS target_orders, (SELECT COUNT(*) FROM source_orders s LEFT JOIN target_orders t ON s.order_id=t.order_id WHERE t.order_id IS NULL) AS unloaded, (SELECT SUM(CAST(COALESCE(revenue,'0') AS REAL)) FROM raw_events WHERE LOWER(status)='completed') AS total_event_revenue, (SELECT COUNT(*) FROM raw_events WHERE LOWER(status)='failed') AS failed_events;",
  ],
  xpAward: 300,
};
