...
export const bi020: Mission = {
  id: "bi-ticket-020",
  world: "AstraMind Analytics",
  levelLabel: "Ticket BI-020 // Priority: High",
  npc: "VP OF ANALYTICS KRISHNA MEHTA",
  briefing: [
    "11:00 AM. Internal system. Priority: High.",
    "\"Dimension slicing — filter the fact set by a dimension attribute and only then aggregate. Enterprise customers only: which products did they spend most on?\"",
  ],
  objective:
    "For Enterprise customers only, show each product's total revenue, sorted descending.",
  ...
  expectedColumns: ["product_name", "enterprise_revenue"],
  expectedRows: [
    ["StreamFlow", 39000],
    ["DashKit", 21700],
    ["ReportBuilder", 19800],
  ...
  hints: [
    "Join both dimensions, filter with WHERE c.segment = 'Enterprise' before any aggregation, then group by product name.",
    "The WHERE applies after the JOIN but before the GROUP BY — the filter reduces the fact rows, then the aggregate summarises what's left.",
    "Try: SELECT p.product_name, SUM(f.amount) AS enterprise_revenue FROM fact_sales f JOIN dim_product p ON f.product_key=p.product_key JOIN dim_customer c ON f.customer_key=c.customer_key WHERE c.segment='Enterprise' GROUP BY p.product_name ORDER BY enterprise_revenue DESC;",
  ],
  xpAward: 250,
};
