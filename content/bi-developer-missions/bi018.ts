...
export const bi018: Mission = {
  id: "bi-ticket-018",
  world: "AstraMind Analytics",
  levelLabel: "Ticket BI-018 // Priority: Medium",
  npc: "VP OF ANALYTICS KRISHNA MEHTA",
  briefing: [
    "09:40 AM. Internal system. Priority: Medium.",
    "\"Second dimension — sales by customer segment. How many transactions did Enterprise vs SMB generate, and what did each segment spend?\"",
  ],
  objective:
    "Join fact_sales to dim_customer and show each segment's transaction count and revenue, sorted by revenue descending.",
  ...
  expectedColumns: ["segment", "transactions", "revenue"],
  expectedRows: [
    ["Enterprise", 13, 111600],
    ["SMB", 11, 103800],
  ],
  requireRowOrder: true,
  hints: [
    "Same shape as the last ticket — just swap dim_product for dim_customer and product_key for customer_key.",
    "Try: SELECT c.segment, COUNT(*) AS transactions, SUM(f.amount) AS revenue FROM fact_sales f JOIN dim_customer c ON f.customer_key=c.customer_key GROUP BY c.segment ORDER BY revenue DESC;",
  ],
  xpAward: 200,
};
