...
export const bi039: Mission = {
  id: "bi-ticket-039",
  world: "AstraMind Analytics",
  levelLabel: "Ticket BI-039 // Priority: High",
  npc: "VP OF ANALYTICS KRISHNA MEHTA",
  briefing: [
    "10:20 AM. Internal system. Priority: High.",
    "\"Regional scorecard — four KPIs per region: deal count, revenue, average deal size, and share of total. One region per row, sorted by revenue.\"",
  ],
  objective:
    "For each customer region, show deal count, total revenue, average deal value (0 decimals), and percentage of total revenue (1 decimal), sorted by revenue descending.",
  ...
  expectedColumns: ["region", "deals", "revenue", "avg_deal", "pct_of_total"],
  expectedRows: [
    ["North", 6, 57100, 9517, 26.5],
    ["West", 7, 54500, 7786, 25.3],
    ["South", 5, 52300, 10460, 24.3],
    ["East", 6, 51500, 8583, 23.9],
  ],
  requireRowOrder: true,
  hints: [
    "Join fact_sales to dim_customer for the region, then GROUP BY region for the aggregates. Add SUM(SUM(f.amount)) OVER () for the grand total to compute the share.",
    "Try: SELECT c.region, COUNT(*) AS deals, SUM(f.amount) AS revenue, ROUND(AVG(f.amount),0) AS avg_deal, ROUND(100.0*SUM(f.amount)/SUM(SUM(f.amount)) OVER (),1) AS pct_of_total FROM fact_sales f JOIN dim_customer c ON f.customer_key=c.customer_key GROUP BY c.region ORDER BY revenue DESC;",
  ],
  xpAward: 250,
};
