...
export const bi046: Mission = {
  id: "bi-ticket-046",
  world: "AstraMind Analytics",
  levelLabel: "Ticket BI-046 // Final Assessment 2 of 6",
  npc: "VP OF ANALYTICS KRISHNA MEHTA",
  briefing: [
    "10:00 AM. Internal system. Final Assessment.",
    "\"Product category revenue matrix — Enterprise vs SMB columns, category rows, total, and each category's share of the company. Three modules in one query: star schema join, pivot, ratio.\"",
  ],
  objective:
    "For each product category, show Enterprise revenue, SMB revenue, total revenue, and category's percentage of total company revenue (1 decimal), sorted by total descending.",
  ...
  expectedColumns: ["category", "enterprise", "smb", "total", "pct_of_company"],
  expectedRows: [
    ["Data Engineering", 47000, 45000, 92000, 42.7],
    ["Analytics", 23100, 52200, 75300, 35],
    ["Visualisation", 41500, 6600, 48100, 22.3],
  ],
  requireRowOrder: true,
  hints: [
    "Two dimension JOINs for the star schema, conditional SUM for the segment pivot, SUM(SUM()) OVER () for the company-total denominator.",
    "Try: SELECT p.category, SUM(CASE WHEN c.segment='Enterprise' THEN f.amount ELSE 0 END) AS enterprise, SUM(CASE WHEN c.segment='SMB' THEN f.amount ELSE 0 END) AS smb, SUM(f.amount) AS total, ROUND(100.0*SUM(f.amount)/SUM(SUM(f.amount)) OVER (),1) AS pct_of_company FROM fact_sales f JOIN dim_product p ON f.product_key=p.product_key JOIN dim_customer c ON f.customer_key=c.customer_key GROUP BY p.category ORDER BY total DESC;",
  ],
  xpAward: 300,
};
