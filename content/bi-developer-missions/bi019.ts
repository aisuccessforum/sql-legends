...
export const bi019: Mission = {
  id: "bi-ticket-019",
  world: "AstraMind Analytics",
  levelLabel: "Ticket BI-019 // Priority: High",
  npc: "VP OF ANALYTICS KRISHNA MEHTA",
  briefing: [
    "10:20 AM. Internal system. Priority: High.",
    "\"Two dimensions at once — the full cross-tab of category vs segment. Join both dimension tables to the fact table, then GROUP BY on attributes from each.\"",
  ],
  objective:
    "Join fact_sales to both dimensions and show revenue grouped by product category and customer segment, sorted by category then revenue descending.",
  ...
  expectedColumns: ["category", "segment", "revenue"],
  expectedRows: [
    ["Analytics", "SMB", 52200],
    ["Analytics", "Enterprise", 23100],
    ["Data Engineering", "Enterprise", 47000],
    ["Data Engineering", "SMB", 45000],
    ["Visualisation", "Enterprise", 41500],
    ["Visualisation", "SMB", 6600],
  ],
  requireRowOrder: true,
  hints: [
    "Two JOIN clauses — one to dim_product on product_key, one to dim_customer on customer_key — then GROUP BY on one attribute from each.",
    "Try: SELECT p.category, c.segment, SUM(f.amount) AS revenue FROM fact_sales f JOIN dim_product p ON f.product_key=p.product_key JOIN dim_customer c ON f.customer_key=c.customer_key GROUP BY p.category, c.segment ORDER BY p.category, revenue DESC;",
  ],
  xpAward: 250,
};
