...
export const bi032: Mission = {
  id: "bi-ticket-032",
  world: "AstraMind Analytics",
  levelLabel: "Ticket BI-032 // Priority: High",
  npc: "VP OF ANALYTICS KRISHNA MEHTA",
  briefing: [
    "09:45 AM. Internal system. Priority: High.",
    "\"Layer two — add a YTD cumulative column on top of the base. The second CTE reads the first CTE's output and adds the window function. Each layer stays clean.\"",
  ],
  objective:
    "Add a second CTE layer that computes the running YTD total on top of the monthly base, sorted chronologically.",
  ...
  expectedColumns: ["month", "revenue", "ytd"],
  expectedRows: [
    ["2026-01", 23000, 23000],
    ["2026-02", 24100, 47100],
    ["2026-03", 30000, 77100],
    ["2026-04", 24400, 101500],
    ["2026-05", 34200, 135700],
    ["2026-06", 32800, 168500],
    ["2026-07", 18300, 186800],
    ["2026-08", 28600, 215400],
  ],
  requireRowOrder: true,
  hints: [
    "The second CTE references the first by name — WITH base AS (...), with_running AS (SELECT ..., SUM(revenue) OVER (...) FROM base).",
    "Try: WITH base AS (SELECT strftime('%Y-%m', sale_date) AS month, SUM(amount) AS revenue FROM fact_sales GROUP BY month), with_running AS (SELECT month, revenue, SUM(revenue) OVER (ORDER BY month) AS ytd FROM base) SELECT month, revenue, ytd FROM with_running ORDER BY month;",
  ],
  xpAward: 225,
};
