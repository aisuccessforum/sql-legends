...
export const bi043: Mission = {
  id: "bi-ticket-043",
  world: "AstraMind Analytics",
  levelLabel: "Ticket BI-043 // Priority: Critical",
  npc: "VP OF ANALYTICS KRISHNA MEHTA",
  briefing: [
    "01:00 PM. Internal system. Priority: Critical.",
    "\"Top customer per month — a monthly leaderboard. For each month, which customer spent the most? The pattern: aggregate revenue per month+customer, then window-rank within month, then filter to rank 1.\"",
  ],
  objective:
    "For each month, show the top-spending customer's name and revenue, sorted chronologically.",
  ...
  expectedColumns: ["month", "customer_name", "monthly_revenue"],
  expectedRows: [
    ["2026-01", "Quantum Corp", 8000],
    ["2026-02", "DataFirst", 13000],
    ["2026-03", "Metro Analytics", 12400],
    ["2026-04", "Bright Ideas Ltd", 13500],
    ["2026-05", "DataFirst", 12000],
    ["2026-06", "Quantum Corp", 19500],
    ["2026-07", "DataFirst", 7200],
    ["2026-08", "Metro Analytics", 6600],
  ],
  requireRowOrder: true,
  hints: [
    "WITH monthly_customer AS (aggregate per month+customer), add RANK() OVER (PARTITION BY month ORDER BY revenue DESC), then filter to rank 1 in the outer query.",
    "Try: WITH monthly_customer AS (SELECT strftime('%Y-%m', f.sale_date) AS month, c.customer_name, SUM(f.amount) AS monthly_revenue, RANK() OVER (PARTITION BY strftime('%Y-%m', f.sale_date) ORDER BY SUM(f.amount) DESC) AS rnk FROM fact_sales f JOIN dim_customer c ON f.customer_key=c.customer_key GROUP BY month, c.customer_name) SELECT month, customer_name, monthly_revenue FROM monthly_customer WHERE rnk=1 ORDER BY month;",
  ],
  xpAward: 275,
};
