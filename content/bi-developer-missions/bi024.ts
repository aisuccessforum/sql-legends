...
export const bi024: Mission = {
  id: "bi-ticket-024",
  world: "AstraMind Analytics",
  levelLabel: "Ticket BI-024 // Priority: Critical",
  npc: "VP OF ANALYTICS KRISHNA MEHTA",
  briefing: [
    "01:40 PM. Internal system. Priority: Critical.",
    "\"Module capstone — the executive product-segment matrix. Category rows, Enterprise vs SMB as columns, total and Enterprise share. This is the report that goes to the board every month.\"",
  ],
  objective:
    "For each product category, show Enterprise revenue, SMB revenue, total revenue, and Enterprise's percentage of that category's total, sorted by total descending.",
  ...
  expectedColumns: ["category", "enterprise", "smb", "total", "enterprise_pct"],
  expectedRows: [
    ["Data Engineering", 47000, 45000, 92000, 51.1],
    ["Analytics", 23100, 52200, 75300, 30.7],
    ["Visualisation", 41500, 6600, 48100, 86.3],
  ],
  requireRowOrder: true,
  hints: [
    "Conditional SUM for Enterprise and SMB columns (pivot pattern), plain SUM for total, ratio of Enterprise to total for the percentage.",
    "Try: SELECT p.category, SUM(CASE WHEN c.segment='Enterprise' THEN f.amount ELSE 0 END) AS enterprise, SUM(CASE WHEN c.segment='SMB' THEN f.amount ELSE 0 END) AS smb, SUM(f.amount) AS total, ROUND(100.0*SUM(CASE WHEN c.segment='Enterprise' THEN f.amount ELSE 0 END)/SUM(f.amount),1) AS enterprise_pct FROM fact_sales f JOIN dim_product p ON f.product_key=p.product_key JOIN dim_customer c ON f.customer_key=c.customer_key GROUP BY p.category ORDER BY total DESC;",
  ],
  xpAward: 300,
};
