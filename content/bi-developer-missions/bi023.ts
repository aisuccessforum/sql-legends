...
export const bi023: Mission = {
  id: "bi-ticket-023",
  world: "AstraMind Analytics",
  levelLabel: "Ticket BI-023 // Priority: High",
  npc: "VP OF ANALYTICS KRISHNA MEHTA",
  briefing: [
    "01:00 PM. Internal system. Priority: High.",
    "\"Top customer per region — one row per region, the name and total of whoever spent the most there. A BI staple: 'who's the whale in each territory?'\"",
  ],
  objective:
    "For each region, show the customer name and revenue of the top-spending customer, sorted by region.",
  ...
  expectedColumns: ["region", "customer_name", "revenue"],
  expectedRows: [
    ["East", "DataFirst", 33000],
    ["North", "Quantum Corp", 31900],
    ["South", "Bright Ideas Ltd", 27300],
    ["West", "Apex Innovations", 40400],
  ],
  requireRowOrder: true,
  hints: [
    "RANK() OVER (PARTITION BY region ORDER BY SUM(amount) DESC) inside a CTE — but window functions can't wrap aggregates directly in GROUP BY queries, so aggregate first in the CTE, then window.",
    "Try: WITH ranked AS (SELECT c.region, c.customer_name, SUM(f.amount) AS revenue, RANK() OVER (PARTITION BY c.region ORDER BY SUM(f.amount) DESC) AS rnk FROM fact_sales f JOIN dim_customer c ON f.customer_key=c.customer_key GROUP BY c.region, c.customer_name) SELECT region, customer_name, revenue FROM ranked WHERE rnk=1 ORDER BY region;",
  ],
  xpAward: 250,
};
