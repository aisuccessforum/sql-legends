...
export const bi035: Mission = {
  id: "bi-ticket-035",
  world: "AstraMind Analytics",
  levelLabel: "Ticket BI-035 // Priority: High",
  npc: "VP OF ANALYTICS KRISHNA MEHTA",
  briefing: [
    "12:00 PM. Internal system. Priority: High.",
    "\"Dimensional layer — region breakdown with category share. Base layer is region + category revenue from the star schema; the second layer adds each category's share of its region's total; the third layer adds a rank within region.\"",
  ],
  objective:
    "Build a 3-layer CTE: region+category revenue, category share of region total, rank within region — all columns, sorted by region then revenue descending.",
  ...
  expectedColumns: ["region", "category", "revenue", "pct_of_region", "rank_in_region"],
  expectedRows: [
    ["East", "Data Engineering", 29000, 56.3, 1],
    ["East", "Analytics", 22500, 43.7, 2],
    ["North", "Visualisation", 29600, 51.8, 1],
    ["North", "Data Engineering", 27500, 48.2, 2],
    ["South", "Analytics", 29700, 56.8, 1],
    ["South", "Data Engineering", 16000, 30.6, 2],
    ["South", "Visualisation", 6600, 12.6, 3],
    ["West", "Analytics", 23100, 42.4, 1],
    ["West", "Data Engineering", 19500, 35.8, 2],
    ["West", "Visualisation", 11900, 21.8, 3],
  ],
  requireRowOrder: true,
  hints: [
    "Base layer: star schema join grouped by region + category. Second layer: add SUM(revenue) OVER (PARTITION BY region) for the regional total, divide for share. Third: RANK() OVER (PARTITION BY region ORDER BY revenue DESC).",
    "Try: WITH base AS (SELECT c.region, p.category, SUM(f.amount) AS revenue FROM fact_sales f JOIN dim_product p ON f.product_key=p.product_key JOIN dim_customer c ON f.customer_key=c.customer_key GROUP BY c.region, p.category), with_share AS (SELECT region, category, revenue, ROUND(100.0*revenue/SUM(revenue) OVER (PARTITION BY region),1) AS pct_of_region FROM base), with_rank AS (SELECT region, category, revenue, pct_of_region, RANK() OVER (PARTITION BY region ORDER BY revenue DESC) AS rank_in_region FROM with_share) SELECT region, category, revenue, pct_of_region, rank_in_region FROM with_rank ORDER BY region, revenue DESC;",
  ],
  xpAward: 275,
};
