...
export const bi049: Mission = {
  id: "bi-ticket-049",
  world: "AstraMind Analytics",
  levelLabel: "Ticket BI-049 // Final Assessment 5 of 6",
  npc: "VP OF ANALYTICS KRISHNA MEHTA",
  briefing: [
    "01:00 PM. Internal system. Final Assessment.",
    "\"Regional YTD layered on a star schema JOIN — monthly revenue per region with its running YTD. The region comes from the customer dimension, not the fact table. Two layers: base aggregation with the JOIN, YTD window on top.\"",
  ],
  objective:
    "For each region and month with sales, show monthly revenue and the region's YTD cumulative total, sorted by region then month.",
  ...
  hints: [
    "First CTE: JOIN fact_sales to dim_customer, group by region + month to get monthly revenue per region. Second CTE: add SUM(revenue) OVER (PARTITION BY region ORDER BY month) for the regional YTD.",
    "Try: WITH region_month AS (SELECT c.region, strftime('%Y-%m', f.sale_date) AS month, SUM(f.amount) AS revenue FROM fact_sales f JOIN dim_customer c ON f.customer_key=c.customer_key GROUP BY c.region, month), with_ytd AS (SELECT region, month, revenue, SUM(revenue) OVER (PARTITION BY region ORDER BY month) AS regional_ytd FROM region_month) SELECT region, month, revenue, regional_ytd FROM with_ytd ORDER BY region, month;",
  ],
  xpAward: 300,
};
