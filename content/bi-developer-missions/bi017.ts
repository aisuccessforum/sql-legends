...
export const bi017: Mission = {
  id: "bi-ticket-017",
  world: "AstraMind Analytics",
  levelLabel: "Ticket BI-017 // Priority: High",
  npc: "VP OF ANALYTICS KRISHNA MEHTA",
  briefing: [
    "09:00 AM. Internal system. Priority: High.",
    "\"New module — star schema. This is how data warehouses are actually structured: one fact table recording what happened, dimension tables recording what things are. You query the fact table and JOIN in the dimensions to add context.\"",
    "\"Start with one dimension — total units sold and revenue per product name, sorted by revenue.\"",
  ],
  objective:
    "Join fact_sales to dim_product and show each product's total units sold and revenue, sorted by revenue descending.",
  ...
  expectedColumns: ["product_name", "units_sold", "revenue"],
  expectedRows: [
    ["StreamFlow", 8, 52000],
    ["DataVault Pro", 9, 40500],
    ["PipelineX", 5, 40000],
    ["QueryMaster", 29, 34800],
    ["ReportBuilder", 12, 26400],
    ["DashKit", 7, 21700],
  ],
  requireRowOrder: true,
  hints: [
    "The fact table has the numbers; dim_product has the names. JOIN on product_key.",
    "GROUP BY the dimension attribute (product_name) after the join — the fact table's keys are surrogate integers, not what you group on.",
    "Try: SELECT p.product_name, SUM(f.quantity) AS units_sold, SUM(f.amount) AS revenue FROM fact_sales f JOIN dim_product p ON f.product_key=p.product_key GROUP BY p.product_name ORDER BY revenue DESC;",
  ],
  xpAward: 225,
};
