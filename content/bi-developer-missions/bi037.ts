...
export const bi037: Mission = {
  id: "bi-ticket-037",
  world: "AstraMind Analytics",
  levelLabel: "Ticket BI-037 // Priority: High",
  npc: "VP OF ANALYTICS KRISHNA MEHTA",
  briefing: [
    "09:00 AM. Internal system. Priority: High.",
    "\"New module — executive dashboards. The CEO gets one row. Everything on it. Start with the four top-line KPIs: total revenue, transaction count, average deal size, and total units sold.\"",
  ],
  objective:
    "In a single row, show total revenue, total transactions, average transaction value (rounded to 0 decimals), and total units sold.",
  ...
  expectedColumns: ["total_revenue", "total_transactions", "avg_transaction", "total_units"],
  expectedRows: [
    [215400, 24, 8975, 70],
  ],
  requireRowOrder: false,
  hints: [
    "No GROUP BY — the whole fact table collapses to one row. Four aggregate functions, no window functions needed.",
    "Try: SELECT SUM(amount) AS total_revenue, COUNT(*) AS total_transactions, ROUND(AVG(amount),0) AS avg_transaction, SUM(quantity) AS total_units FROM fact_sales;",
  ],
  xpAward: 200,
};
