...
export const bi033: Mission = {
  id: "bi-ticket-033",
  world: "AstraMind Analytics",
  levelLabel: "Ticket BI-033 // Priority: Medium",
  npc: "VP OF ANALYTICS KRISHNA MEHTA",
  briefing: [
    "10:30 AM. Internal system. Priority: Medium.",
    "\"Layer three — ranking. Add a revenue_rank to the layered output, then filter the final SELECT to only the top 3 revenue months. Three layers, one focused output.\"",
  ],
  objective:
    "Add a ranking layer to the previous two-layer CTE and return only the top 3 months by revenue with their YTD totals.",
  ...
  expectedColumns: ["month", "revenue", "ytd", "revenue_rank"],
  expectedRows: [
    ["2026-05", 34200, 135700, 1],
    ["2026-06", 32800, 168500, 2],
    ["2026-03", 30000, 77100, 3],
  ],
  requireRowOrder: true,
  hints: [
    "The third CTE adds RANK() OVER (ORDER BY revenue DESC) on top of the with_running CTE output.",
    "Filter to revenue_rank <= 3 in the outer SELECT — you can't filter on window functions directly in WHERE.",
    "Try: WITH base AS (SELECT strftime('%Y-%m', sale_date) AS month, SUM(amount) AS revenue FROM fact_sales GROUP BY month), with_running AS (SELECT month, revenue, SUM(revenue) OVER (ORDER BY month) AS ytd FROM base), with_rank AS (SELECT month, revenue, ytd, RANK() OVER (ORDER BY revenue DESC) AS revenue_rank FROM with_running) SELECT month, revenue, ytd, revenue_rank FROM with_rank WHERE revenue_rank<=3 ORDER BY revenue_rank;",
  ],
  xpAward: 225,
};
