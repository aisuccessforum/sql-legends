...
export const bi041: Mission = {
  id: "bi-ticket-041",
  world: "AstraMind Analytics",
  levelLabel: "Ticket BI-041 // Priority: High",
  npc: "VP OF ANALYTICS KRISHNA MEHTA",
  briefing: [
    "11:40 AM. Internal system. Priority: High.",
    "\"Customer segment health — Enterprise vs SMB. Unique customers, transaction count, total revenue, average transaction, and transactions per customer. Tells you which segment is more valuable per head.\"",
  ],
  objective:
    "For each customer segment, show unique customer count, transaction count, revenue, average transaction (0 decimals), and transactions per customer (1 decimal), sorted by revenue descending.",
  ...
  expectedColumns: ["segment", "customers", "transactions", "revenue", "avg_transaction", "txn_per_customer"],
  expectedRows: [
    ["Enterprise", 4, 13, 111600, 8585, 3.3],
    ["SMB", 4, 11, 103800, 9436, 2.8],
  ],
  requireRowOrder: true,
  hints: [
    "COUNT(DISTINCT f.customer_key) for unique customers; the rest are standard aggregates over the joined table.",
    "Try: SELECT c.segment, COUNT(DISTINCT f.customer_key) AS customers, COUNT(*) AS transactions, SUM(f.amount) AS revenue, ROUND(AVG(f.amount),0) AS avg_transaction, ROUND(1.0*COUNT(*)/COUNT(DISTINCT f.customer_key),1) AS txn_per_customer FROM fact_sales f JOIN dim_customer c ON f.customer_key=c.customer_key GROUP BY c.segment ORDER BY revenue DESC;",
  ],
  xpAward: 250,
};
