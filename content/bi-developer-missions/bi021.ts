...
export const bi021: Mission = {
  id: "bi-ticket-021",
  world: "AstraMind Analytics",
  levelLabel: "Ticket BI-021 // Priority: High",
  npc: "VP OF ANALYTICS KRISHNA MEHTA",
  briefing: [
    "11:40 AM. Internal system. Priority: High.",
    "\"Drill-down — category to product. Revenue by category, then within each category show each product's share. This is the BI 'expand row' interaction in SQL form.\"",
  ],
  objective:
    "For each category, show each product's revenue and its percentage of that category's total, sorted by category then revenue descending.",
  ...
  expectedColumns: ["category", "product_name", "revenue", "pct_of_category"],
  expectedRows: [
    ["Analytics", "DataVault Pro", 40500, 53.8],
    ["Analytics", "QueryMaster", 34800, 46.2],
    ["Data Engineering", "StreamFlow", 52000, 56.5],
    ["Data Engineering", "PipelineX", 40000, 43.5],
    ["Visualisation", "ReportBuilder", 26400, 54.9],
    ["Visualisation", "DashKit", 21700, 45.1],
  ],
  requireRowOrder: true,
  hints: [
    "SUM(f.amount) OVER (PARTITION BY p.category) gives the category total on every row — divide each product's revenue by that for the percentage.",
    "Try: SELECT p.category, p.product_name, SUM(f.amount) AS revenue, ROUND(100.0*SUM(f.amount)/SUM(SUM(f.amount)) OVER (PARTITION BY p.category),1) AS pct_of_category FROM fact_sales f JOIN dim_product p ON f.product_key=p.product_key GROUP BY p.category, p.product_name ORDER BY p.category, revenue DESC;",
  ],
  xpAward: 275,
};
