...
export const bi036: Mission = {
  id: "bi-ticket-036",
  world: "AstraMind Analytics",
  levelLabel: "Ticket BI-036 // Priority: Critical",
  npc: "VP OF ANALYTICS KRISHNA MEHTA",
  briefing: [
    "12:45 PM. Internal system. Priority: Critical.",
    "\"Module capstone — six-column layered monthly report. Month, revenue, YTD, MoM growth %, share of total, revenue rank. Six columns, four CTE layers, one clean outer SELECT. This is the full template every BI team keeps in their library.\"",
  ],
  objective:
    "Build the complete 4-layer CTE report: month, revenue, YTD, MoM %, share of total, and revenue rank — all sorted chronologically.",
  ...
  expectedColumns: ["month", "revenue", "ytd", "mom_pct", "pct_of_total", "revenue_rank"],
  expectedRows: [
    ["2026-01", 23000, 23000, null, 10.7, 7],
    ["2026-02", 24100, 47100, 4.8, 11.2, 6],
    ["2026-03", 30000, 77100, 24.5, 13.9, 3],
    ["2026-04", 24400, 101500, -18.7, 11.3, 5],
    ["2026-05", 34200, 135700, 40.2, 15.9, 1],
    ["2026-06", 32800, 168500, -4.1, 15.2, 2],
    ["2026-07", 18300, 186800, -44.2, 8.5, 8],
    ["2026-08", 28600, 215400, 56.3, 13.3, 4],
  ],
  requireRowOrder: true,
  hints: [
    "Extend Ticket BI-034's four layers by adding a fifth CTE that ranks by revenue — or add the RANK() to the share layer since it only reads from the growth layer.",
    "Try: WITH base AS (SELECT strftime('%Y-%m', sale_date) AS month, SUM(amount) AS revenue FROM fact_sales GROUP BY month), with_running AS (SELECT month, revenue, SUM(revenue) OVER (ORDER BY month) AS ytd FROM base), with_growth AS (SELECT month, revenue, ytd, ROUND(100.0*(revenue-LAG(revenue) OVER (ORDER BY month))/NULLIF(LAG(revenue) OVER (ORDER BY month),0),1) AS mom_pct FROM with_running), with_share AS (SELECT month, revenue, ytd, mom_pct, ROUND(100.0*revenue/SUM(revenue) OVER (),1) AS pct_of_total, RANK() OVER (ORDER BY revenue DESC) AS revenue_rank FROM with_growth) SELECT month, revenue, ytd, mom_pct, pct_of_total, revenue_rank FROM with_share ORDER BY month;",
  ],
  xpAward: 300,
};
