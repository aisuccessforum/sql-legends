...
export const bi040: Mission = {
  id: "bi-ticket-040",
  world: "AstraMind Analytics",
  levelLabel: "Ticket BI-040 // Priority: High",
  npc: "VP OF ANALYTICS KRISHNA MEHTA",
  briefing: [
    "11:00 AM. Internal system. Priority: High.",
    "\"Product performance scorecard — units, revenue, effective price (revenue ÷ units), and revenue rank. Six rows, one per product.\"",
  ],
  objective:
    "For each product, show total units sold, total revenue, effective price per unit (0 decimals), and its revenue rank, sorted by revenue descending.",
  ...
  expectedColumns: ["product_name", "category", "units", "revenue", "effective_price", "revenue_rank"],
  expectedRows: [
    ["StreamFlow", "Data Engineering", 8, 52000, 6500, 1],
    ["DataVault Pro", "Analytics", 9, 40500, 4500, 2],
    ["PipelineX", "Data Engineering", 5, 40000, 8000, 3],
    ["QueryMaster", "Analytics", 29, 34800, 1200, 4],
    ["ReportBuilder", "Visualisation", 12, 26400, 2200, 5],
    ["DashKit", "Visualisation", 7, 21700, 3100, 6],
  ],
  requireRowOrder: true,
  hints: [
    "Join to dim_product, GROUP BY product name and category, add RANK() OVER (ORDER BY SUM(amount) DESC) for the rank column.",
    "effective_price = ROUND(SUM(amount)*1.0/SUM(quantity),0) — multiply by 1.0 to avoid integer division.",
    "Try: SELECT p.product_name, p.category, SUM(f.quantity) AS units, SUM(f.amount) AS revenue, ROUND(SUM(f.amount)*1.0/SUM(f.quantity),0) AS effective_price, RANK() OVER (ORDER BY SUM(f.amount) DESC) AS revenue_rank FROM fact_sales f JOIN dim_product p ON f.product_key=p.product_key GROUP BY p.product_name, p.category ORDER BY revenue DESC;",
  ],
  xpAward: 250,
};
