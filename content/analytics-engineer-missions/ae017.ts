...
export const ae017: Mission = {
  id: "ae-ticket-017",
  world: "AstraMind Analytics",
  levelLabel: "Ticket AE-017 // Priority: High",
  npc: "DATA PLATFORM LEAD ARYAN KAPOOR",
  briefing: [
    "09:00 AM. Internal system. Priority: High.",
    "\"New module — mart models. The mart is the final layer: pre-aggregated, business-named, query-ready. Finance, Sales, Product all read from their own mart. Start with the monthly revenue mart.\"",
  ],
  objective:
    "Build a monthly revenue mart: month, total revenue, and count of confirmed orders per month — sorted chronologically.",
  ...
  expectedColumns: ["month", "revenue", "confirmed_orders"],
  expectedRows: [
    ["2026-01", 23000, 2],
    ["2026-02", 24100, 2],
    ["2026-03", 30000, 2],
  ...
  hints: [
    "GROUP BY strftime('%Y-%m', order_date). Use COUNT(CASE WHEN status='confirmed' THEN 1 END) for confirmed-only count alongside the full SUM.",
    "Try: SELECT strftime('%Y-%m',order_date) AS month, SUM(revenue) AS revenue, COUNT(CASE WHEN status='confirmed' THEN 1 END) AS confirmed_orders FROM stg_orders GROUP BY month ORDER BY month;",
  ],
  xpAward: 225,
};
