...
export const ae047: Mission = {
  id: "ae-ticket-047",
  world: "AstraMind Analytics",
  levelLabel: "Ticket AE-047 // Final Assessment 5 of 8",
  npc: "DATA PLATFORM LEAD ARYAN KAPOOR",
  briefing: [
    "01:00 PM. Internal system. Final Assessment.",
    "\"3-CTE pipeline — stg_confirmed → int_customer → fct_segment. Customer-level aggregation rolled up to segment level with gross margin. Named conventions, no raw table access after stg_.\"",
  ],
  objective:
    "Using three CTEs (stg_confirmed, int_customer, fct_segment), show each segment's customer count, total orders, revenue, gross margin, and margin percentage — sorted by revenue descending.",
  ...
  expectedColumns: ["segment", "customers", "total_orders", "revenue", "gross_margin", "margin_pct"],
  expectedRows: [
    ["SMB", 4, 6, 69100, 23400, 33.9],
    ["Enterprise", 4, 7, 55300, 18800, 34],
  ],
  requireRowOrder: true,
  hints: [
    "stg_confirmed: join stg_orders + stg_products, WHERE confirmed, keep customer_id + revenue + unit_price + cost + qty. int_customer: GROUP BY customer_id for orders, revenue, gross_margin. fct_segment: JOIN int_customer to stg_customers, GROUP BY segment.",
    "Try: WITH stg_confirmed AS (SELECT o.customer_id, o.revenue, o.unit_price, p.cost, o.qty FROM stg_orders o JOIN stg_products p ON o.product_id=p.product_id WHERE o.status='confirmed'), int_customer AS (SELECT customer_id, SUM(revenue) AS revenue, SUM((unit_price-cost)*qty) AS gross_margin, COUNT(*) AS orders FROM stg_confirmed GROUP BY customer_id), fct_segment AS (SELECT c.segment, COUNT(*) AS customers, SUM(ic.orders) AS total_orders, SUM(ic.revenue) AS revenue, SUM(ic.gross_margin) AS gross_margin, ROUND(100.0*SUM(ic.gross_margin)/SUM(ic.revenue),1) AS margin_pct FROM int_customer ic JOIN stg_customers c ON ic.customer_id=c.customer_id GROUP BY c.segment) SELECT segment, customers, total_orders, revenue, gross_margin, margin_pct FROM fct_segment ORDER BY revenue DESC;",
  ],
  xpAward: 300,
};
