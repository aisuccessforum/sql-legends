...
export const ae012: Mission = {
  id: "ae-ticket-012",
  world: "AstraMind Analytics",
  levelLabel: "Ticket AE-012 // Priority: High",
  npc: "DATA PLATFORM LEAD ARYAN KAPOOR",
  briefing: [
    "11:15 AM. Internal system. Priority: High.",
    "\"Gross margin per order — the most important enrichment you add in the intermediate layer. Unit margin × qty gives you gross margin in dollar terms; gross_margin/revenue gives you margin rate.\"",
  ],
  objective:
    "For each order, show order_id, product_name, revenue, gross margin ((unit_price-cost)×qty rounded to 2 decimal), and margin percentage (1 decimal), sorted by order_id.",
  ...
  expectedColumns: ["order_id", "product_name", "revenue", "gross_margin", "margin_pct"],
  expectedRows: [
    ["ORD-001", "DataVault Pro", 9000, 2600, 28.9],
    ["ORD-002", "QueryMaster", 6000, 2500, 41.7],
    ["ORD-003", "PipelineX", 8000, 2500, 31.3],
    ["ORD-004", "ReportBuilder", 6600, 2400, 36.4],
    ["ORD-005", "DataVault Pro", 4500, 1300, 28.9],
    ["ORD-006", "StreamFlow", 13000, 4600, 35.4],
    ["ORD-007", "DashKit", 12400, 4400, 35.5],
    ["ORD-008", "QueryMaster", 9600, 4000, 41.7],
    ["ORD-009", "PipelineX", 8000, 2500, 31.3],
    ["ORD-010", "ReportBuilder", 4400, 1600, 36.4],
    ["ORD-011", "DataVault Pro", 13500, 3900, 28.9],
    ["ORD-012", "StreamFlow", 6500, 2300, 35.4],
    ["ORD-013", "QueryMaster", 12000, 5000, 41.7],
    ["ORD-014", "DashKit", 6200, 2200, 35.5],
    ["ORD-015", "PipelineX", 16000, 5000, 31.3],
    ["ORD-016", "ReportBuilder", 8800, 3200, 36.4],
  ],
  requireRowOrder: true,
  hints: [
    "Gross margin = ROUND((o.unit_price - p.cost) * o.qty, 2). Margin pct = ROUND(100.0*(o.unit_price-p.cost)/o.unit_price, 1).",
    "Try: SELECT o.order_id, p.product_name, o.revenue, ROUND((o.unit_price-p.cost)*o.qty,2) AS gross_margin, ROUND(100.0*(o.unit_price-p.cost)/o.unit_price,1) AS margin_pct FROM stg_orders o JOIN stg_products p ON o.product_id=p.product_id ORDER BY o.order_id;",
  ],
  xpAward: 250,
};
