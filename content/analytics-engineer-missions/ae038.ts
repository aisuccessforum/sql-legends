...
export const ae038: Mission = {
  id: "ae-ticket-038",
  world: "AstraMind Analytics",
  levelLabel: "Ticket AE-038 // Priority: High",
  npc: "DATA PLATFORM LEAD ARYAN KAPOOR",
  briefing: [
    "09:45 AM. Internal system. Priority: High.",
    "\"Remove a repeated subexpression — the old query computed monthly revenue twice to produce a share column. Extract it once into a CTE named 'monthly' and use it in both places.\"",
  ],
  objective:
    "Using a CTE named 'monthly', show each confirmed-order month's revenue, its share of the annual total (1 decimal), and its revenue rank — sorted chronologically.",
  ...
  expectedColumns: ["month", "revenue", "pct_of_total", "rank"],
  expectedRows: [
    ["2026-01", 17000, 13.7, 5],
    ["2026-02", 19600, 15.8, 4],
    ["2026-03", 20400, 16.4, 3],
    ["2026-04", 24400, 19.6, 2],
    ["2026-05", 34200, 27.5, 1],
    ["2026-06", 8800, 7.1, 6],
  ],
  requireRowOrder: true,
  hints: [
    "WITH monthly AS (SELECT strftime, SUM(revenue) ... WHERE confirmed GROUP BY month). In the outer SELECT, SUM(revenue) OVER () gives the grand total; RANK() OVER (ORDER BY revenue DESC) gives rank.",
    "Try: WITH monthly AS (SELECT strftime('%Y-%m',order_date) AS month, SUM(revenue) AS revenue FROM stg_orders WHERE status='confirmed' GROUP BY month) SELECT month, revenue, ROUND(100.0*revenue/SUM(revenue) OVER (),1) AS pct_of_total, RANK() OVER (ORDER BY revenue DESC) AS rank FROM monthly ORDER BY month;",
  ],
  xpAward: 250,
};
