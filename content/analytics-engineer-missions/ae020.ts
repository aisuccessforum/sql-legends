...
export const ae020: Mission = {
  id: "ae-ticket-020",
  world: "AstraMind Analytics",
  levelLabel: "Ticket AE-020 // Priority: High",
  npc: "DATA PLATFORM LEAD ARYAN KAPOOR",
  briefing: [
    "11:15 AM. Internal system. Priority: High.",
    "\"Regional mart — confirmed revenue by region with share of total. Four rows, clean, ready for the geography dashboard.\"",
  ],
  objective:
    "For confirmed orders, show each region's order count, revenue, and percentage of total revenue (1 decimal) — sorted by revenue descending.",
  ...
  expectedColumns: ["region", "orders", "revenue", "pct_of_total"],
  expectedRows: [
    ["South", 3, 36100, 29],
    ["East", 3, 33000, 26.5],
    ["North", 4, 31000, 24.9],
  ...
  hints: [
    "JOIN stg_orders to stg_customers, WHERE status='confirmed', GROUP BY region. SUM(SUM(revenue)) OVER () gives the grand total for the share.",
    "Try: SELECT c.region, COUNT(o.order_id) AS orders, SUM(o.revenue) AS revenue, ROUND(100.0*SUM(o.revenue)/SUM(SUM(o.revenue)) OVER (),1) AS pct_of_total FROM stg_orders o JOIN stg_customers c ON o.customer_id=c.customer_id WHERE o.status='confirmed' GROUP BY c.region ORDER BY revenue DESC;",
  ],
  xpAward: 225,
};
