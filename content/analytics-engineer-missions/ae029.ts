...
export const ae029: Mission = {
  id: "ae-ticket-029",
  world: "AstraMind Analytics",
  levelLabel: "Ticket AE-029 // Priority: High",
  npc: "DATA PLATFORM LEAD ARYAN KAPOOR",
  briefing: [
    "01:30 PM. Internal system. Priority: High.",
    "\"Referential integrity test — every order must reference a product that exists in stg_products. An orphan order (pointing to a missing product) breaks every downstream join. Find any orphan orders.\"",
  ],
  objective:
    "Find any orders in stg_orders that reference a product_id not present in stg_products — return their order_id and product_id.",
  schemaLabel: "stg_orders, stg_customers, stg_products (with one orphan order added)",
  seedSql: `
    ...
    INSERT INTO stg_orders VALUES ('ORD-999','C001','P999',1,1000,'2026-06-20','confirmed',1000);`,
  schemaPreview: [
  ...
  expectedColumns: ["order_id", "product_id"],
  expectedRows: [
    ["ORD-999", "P999"],
  ],
  requireRowOrder: false,
  hints: [
    "LEFT JOIN stg_orders to stg_products on product_id — any row where p.product_id IS NULL after the join is an orphan.",
    "Try: SELECT o.order_id, o.product_id FROM stg_orders o LEFT JOIN stg_products p ON o.product_id=p.product_id WHERE p.product_id IS NULL ORDER BY o.order_id;",
  ],
  xpAward: 225,
};
