...
export const ae027: Mission = {
  id: "ae-ticket-027",
  world: "AstraMind Analytics",
  levelLabel: "Ticket AE-027 // Priority: High",
  npc: "DATA PLATFORM LEAD ARYAN KAPOOR",
  briefing: [
    "12:00 PM. Internal system. Priority: High.",
    "\"Accepted values test — status should only be 'new', 'qualified', or 'closed'. Anything else (like 'archived') is an undocumented value that shouldn't be in the table. Return the invalid values and their counts.\"",
  ],
  objective:
    "Return any status values not in the accepted list ('new', 'qualified', 'closed'), along with how many times each appears.",
  ...
  expectedColumns: ["status", "occurrences"],
  expectedRows: [
    ["archived", 1],
  ],
  requireRowOrder: false,
  hints: [
    "WHERE status NOT IN ('new','qualified','closed'), then GROUP BY status to count occurrences.",
    "Try: SELECT DISTINCT status, COUNT(*) AS occurrences FROM raw_leads WHERE status NOT IN ('new','qualified','closed') GROUP BY status ORDER BY status;",
  ],
  xpAward: 200,
};
