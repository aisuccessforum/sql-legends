...
export const ae044: Mission = {
  id: "ae-ticket-044",
  world: "AstraMind Analytics",
  levelLabel: "Ticket AE-044 // Final Assessment 2 of 8",
  npc: "DATA PLATFORM LEAD ARYAN KAPOOR",
  briefing: [
    "10:00 AM. Internal system. Final Assessment.",
    "\"Referential integrity gate — no order in stg_orders should point to a missing customer. Count the orphans. This number must be 0 before any mart model runs.\"",
  ],
  objective:
    "Count how many stg_orders rows reference a customer_id that does not exist in stg_customers.",
  ...
  expectedColumns: ["orphan_orders"],
  expectedRows: [
    [0],
  ],
  requireRowOrder: false,
  hints: [
    "LEFT JOIN stg_orders to stg_customers on customer_id, filter WHERE c.customer_id IS NULL, COUNT(*).",
    "Try: SELECT COUNT(*) AS orphan_orders FROM stg_orders o LEFT JOIN stg_customers c ON o.customer_id=c.customer_id WHERE c.customer_id IS NULL;",
  ],
  xpAward: 275,
};
